...
static int findDijstraStructByIndex(Vector* _vec, int _graphIndex);
static void Relax(Heap* _heap, DijstraStruct* _fatherDijstraStruct , int _sonIndex , int _arcWeight);


ADTERR Dijkstra(Graph* _graph, size_t _src , int*  distancesArr, int* pathArr ){
	...
	while(  VectorItemsNum(vec) > 0 ){         /*until heap is empty*/
	...
	for(i=0; i < numOfNodes ; i++){
		...
		distancesArr[toGet->s_index] = toGet->s_minDistance;
		pathArr[toGet->s_index] = toGet->s_fatherAtPath;
	}
...
static void Relax(Heap* _heap, DijstraStruct* _fatherDijstraStruct , int _sonIndex , int _arcWeight){
	int newDistance, heapIndex;
	DijstraStruct* sonDijstraStruct;	

	heapIndex = HeapGetIndexOfHeapfromOrig(_heap, _sonIndex); /*  get heap index(struct->index) from graph node index(i)  */  
	
	VectorGet(  HeapGetVector(_heap) , heapIndex , (void**)&sonDijstraStruct  )   ;

	/*protection*/  /*
	if(sonDijstraStruct->s_index != _sonIndex){ 
		heapIndex = findDijstraStructByIndex (HeapGetVector(_heap) , _sonIndex);
		VectorGet(  HeapGetVector(_heap) , heapIndex , (void**)&sonDijstraStruct  )  ;
	}*/
	...
	if( sonDijstraStruct->s_minDistance > newDistance){	
		...
		sonDijstraStruct->s_fatherAtPath = _fatherDijstraStruct->s_index;;
