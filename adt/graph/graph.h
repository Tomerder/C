...
/*----------------------------------------------------------------------------------------*/
/*call dfs for each node src = dst .
	space complexity: o(1)
...
/*Dijkstra algo - finds path and distance from _src to all nodes on WEIGHTED GRAPH
	parameters: 
			_distancesArr = user supplied initialized arr (malloc) - size of nodes number 
			_pathArr      = user supplied initialized arr (malloc) - size of nodes number
							will return the father of each node at the path
	space complexity: o(n)          :  heap
	time complexity: o(n^2*log(n))  :  for every item visit all sons , if minDistance updated for son => fix heap = log(n)
*/
ADTERR Dijkstra(Graph* _graph, size_t _src, int* _distancesArr, int* _pathArr);
