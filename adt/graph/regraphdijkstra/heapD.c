...
static ADTERR Swap(Vector* _vec,size_t _index1 , size_t _index2 , int* _hushMapIndexes );
...
Heap* HeapBuild (Vector* _vec, WeakOrderFunc _WeakOrderFunc){
	...
	hushMapIndexes = (int*)malloc( nItems * sizeof(int));
	if(hushMapIndexes == NULL){
		return NULL;
	}
	...
	heap->m_vector = _vec; 
	heap->m_hushMapIndexes  = hushMapIndexes;  
...
static void Heapify(Heap* _heap, int _index){
	...
	if (indexRight < nItems){  
	 ...
	 /*swap + recursion*/
	if (indexMax != _index){  
     	Swap ( _heap->m_vector, _index , indexMax, _heap->m_hushMapIndexes);
...
void* HeapExtract(Heap* _heap){
	...
	Swap ( _heap->m_vector, TOP_INDEX, nItems - 1 ,  _heap->m_hushMapIndexes);  
...
static void SiftUp(Heap* _heap, int _index){
	...
	if (indexParent != _index){
		...
		if( _heap->m_WeakOrderFunc (itemParent , itemSon) ){ 
			Swap ( _heap->m_vector, _index , indexParent ,  _heap->m_hushMapIndexes);
...
static ADTERR Swap(Vector* _vec,size_t _index1 , size_t _index2 , int* _hushMapIndexes){
	...
	tmpIndex = _hushMapIndexes[_index1] ; 
	_hushMapIndexes[_index1] = _hushMapIndexes[_index2];
	_hushMapIndexes[_index2] = tmpIndex;
