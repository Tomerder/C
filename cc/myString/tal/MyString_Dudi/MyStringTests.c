...
int MyStrcatTest(char* _destination, const char* _source)
{
	char* expected;
	char* output;
	int pass = 1;
	
	expected = (char*)malloc(strlen(_destination)+strlen(_source)+1);
	strcpy(expected, _destination);

	output = MyStrcat(_destination, _source);
	
	if (output != _destination) {
		printf("MyStrcat Test FAILED:\n\tThe return value points to %p instead of %p.\n\n", (void*)output, (void*)_destination);
		return 0;
	}

	strcat(expected, _source);

	if(strcmp(_destination, expected) != 0) {
		printf("MyStrncpy Test FAILED:\n\tThe destination is \"%s\" instead of \"%s\".\n\n", _destination, expected);
		pass = 0;
	}
...
int MyStrstrTest(const char* _str, const char* _substr)
{
	char* output;
	char* expected;
	
	output = MyStrstr(_str, _substr);
	expected = strstr(_str, _substr);
	
	if (output != expected) {
		printf("MyStrstr Test FAILED:\n\tThe output is \"%s\" instead of \"%s\"\n\n", output, expected);		
		return 0;
	}
...
void RunMyStringTests(void)
{
	...
	for (i = 0; i < n; ++i)
	{
		strcpy(str, strings[(i+1)%n]);
		MyStrcatTest(str, strings[i]);
	}
	...
	for (i = 0; i < n; ++i)
	{
		for (j = 0; j < n; ++j)
		{
			MyStrstrTest(strings[i], strings[j]);
		}
	}
}
