...
void ReadFromMsgQ(int _msqid)
{
	Buf bufRead , bufAck , bufEOF;
	...
	strcpy(bufAck.mtext, EOF_SIGN);
	
	PRINT_V("start reading\n");
		
	while(TRUE){
		/*check EOF on differant channel */
		if(args.m_isMsgEOF){  
			usleep(MIN_SLEEP); /*ensure that eof was already sent by producer*/ 	
			if ( msgrcv(_msqid, &bufEOF, sizeof(bufEOF.mtext), bufEOF.mtype, IPC_NOWAIT) != -1) {
		...
		/*reading*/
		if ( (numOfBytesRcv = (msgrcv(_msqid, &bufRead, sizeof(bufRead.mtext), bufRead.mtype, 0))) == -1) {
            perror("msgrcv");
            exit(1);
        }
