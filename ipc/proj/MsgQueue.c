...
enum{OK_E , ERR_FTOK, ERR_SHMGET , ERR_SHMAT , ERR_SHMDT , ERR_SHMCTL , MY_ALLOC_E};
...
int	MsgQueueSnd (MsgQ* _msgQ, void* _buffer, size_t _length)
{
	...
	QueueInsert( GET_QUEUE(_msgQ) , (Item)msgOffset );
	/*----------------------------------*/
...
{
	Msg* msg = NULL;
	int numOfBytesRead = 0 , msgOffset;	
	...
	QueueRemove( GET_QUEUE(_msgQ) , (Item*)(&msgOffset) );
	...
	numOfBytesRead = MIN(msg->m_len, _size) ;
