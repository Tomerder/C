...
int LlistTestPopHead(size_t _size)
{
	...
	errorInt = ( ((City*)data)->m_population != city[_size-1].m_population );
	ListDestroy(list);
	CityFreeArr(city, _size);
...
int ListTestFindFirst(size_t _size)
{
	...
	error += ( ((Student*)(((Node*)itr)->m_data))->m_grade != ((Student*)(data))->m_grade && strcmp(((Student*)(data))->m_name, ((Student*)(((Node*)itr)->m_data))->m_name) );
...
City *MakeCities(size_t* _num)
{
	...
	for(i=0; i<(*_num); ++i)
	{
		...
		if(NULL == (void*)&cities[i]) {
			*_num = i;
			printf("allocation failed, array size is: %d\n", i);
			break;
			}
	}
	return cities;
