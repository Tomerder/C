...
static int 		StudentCompare	(Data _student1, Data _student2);
static int 		StudentFactor	(Data _student, Param _factor);
...
int LlistTestPushHead(size_t _size)
{
	...
	for(i=0; i<_size; ++i) {
		arrInt[i] = i;
		error += ListPushHead(list, (Data)&arrInt[i]);
	}
	if(error) {
		ListDestroy(list);
		return error;
	...
	for(i=0; i<_size; ++i) {
		error += ListPushHead(list, (Data)&city[i]);
	}
...
int LlistTestPushTail(size_t _size)
{
	...
	for(i=0; i<_size; ++i) {
		arrInt[i] = i;
		error += ListPushTail(list, (Data)&arrInt[i]);
	}
	if(error) {
		ListDestroy(list);
		return error;
	...
	for(i=0; i<_size; ++i) {
		error += ListPushTail(list, (Data)&city[i]);
	}
...
int LlistTestPopHead(size_t _size)
{
	...
	for(i=0; i<_size; ++i) {
		arrInt[i] = i;
		ListPushHead(list, (Data)&arrInt[i]);
	}
	
	intPopped = *(int*)ListPopHead(list);
	
	if( intPopped != arrInt[_size-1] ) {
		ListDestroy(list);
		return 1;
	}
	...
	for(i=0; i<_size; ++i) {
		ListPushHead(list, (Data)&city[i]);
	}

	cityPopped = (City*)ListPopHead(list);
	...
	errorInt = ( (*cityPopped).m_population != city[_size-1].m_population );
	ListDestroy(list);
	CityFreeArr(&city, _size);
...
int LlistTestPopTail(size_t _size)
{
	...
	for(i=0; i<_size; ++i) {
		arrInt[i] = i;
		ListPushTail(list, (Data)&arrInt[i]);
	}
	
	intPopped = *(int*)ListPopTail(list);
	...
	for(i=0; i<_size; ++i) {
		ListPushTail(list, (Data)&city[i]);
	}

	cityPopped = (City*)ListPopTail(list);
...
int ListItrTestBeginEnd(size_t _size)
{
	...
	itr = ListBegin(list);
	error += (itr->m_node->m_data != list->m_head.m_next->m_data);
	
	itr = ListEnd(list);
	error += (itr->m_node->m_data != &list->m_tail.m_data);
	...
	itr = ListBegin(list);
	error += (itr->m_node->m_data != list->m_head.m_next);
	
	itr = ListEnd(list);
	error += (itr->m_node->m_data != &list->m_tail);
...
int ListItrTestGet(size_t _size)
{
	...
	itr->m_node = list->m_head.m_next->m_next;
	data = ListGetData(itr);
...
int ListItrTestInsert(size_t _size)
{
	...
	for(i=0; i<4; ++i)
	{
		itr = ListInsertBefore(itr, (Data)&arrInt2[i]);
		data = ListGetData(itr);
	...
	itr = ListBegin(list);
	itr = ListInsertBefore(itr, (Data)&arrInt2[5]);
	data = ListGetData(itr);
...
int ListTestFindFirst(size_t _size)
{
	...
	for(i=0; i<_size; ++i) {
		if(i%2) {
			ListPushTail(list, (Data)&studentArr[i]);
		} else {
			ListPushHead(list, (Data)&studentArr[i]);
		}
	}
	...
	error += ( ((Student*)(itr->m_node->m_data))->m_grade != ((Student*)(data))->m_grade && strcmp(((Student*)(data))->m_name, ((Student*)(itr->m_node->m_data))->m_name) );
	...
	itr = ListFindFirst(ListBegin(list), ListEnd(list), StudentCompare, (Data)&studentToNotFind);
	error += (itr != ListEnd(list));
	
	/* search with _from=_to */
	itr = ListNext(ListNext(ListBegin(list)));
	itr = ListFindFirst(itr, itr, StudentCompare, (Data)&studentToNotFind);
	error += (itr != ListNext(ListNext(ListBegin(list))));
	
	/* search with _from>_to */
	itr = ListFindFirst(ListNext(itr), ListPrev(itr), StudentCompare, (Data)&studentToNotFind);
...
int StudentCompare(Data _student1, Data _student2)
{
...
int StudentFactor(Data _student, void* _factor)
{
