...
int main(int argc, char *argv[])
{
	...
	SendRecvArg eofArgs;	/* arguments to sent EOF */
	...
	semPingVal = semctl(semid, PING, GETVAL);
	if( -1 == semPingVal)
	{
		perror("semctl");
		return errno;
	...
	if( 0 == semPingVal)
	{
		PrintV(&options, "Sending EOF", -1);
		
		if( -1 == MsgSend(&eofArgs, &messegeOut) )
		{
			fprintf(stderr, "error: MsgSnd %d (EOF)\n", pid);
			return 1;
		}
	}
