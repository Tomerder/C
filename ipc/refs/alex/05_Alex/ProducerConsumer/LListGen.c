...
ListItr		ListInsertBefore	(ListItr _itr, ListData _data)
{
	...
	/* pointers from new node to neighbors */
	tempNode->m_prev = ((Node*)_itr)->m_prev;
	tempNode->m_next = (Node*)_itr;
	/* pointers from neighbors to new node */
	((Node*)_itr)->m_prev->m_next = tempNode;
	((Node*)_itr)->m_prev = tempNode;
