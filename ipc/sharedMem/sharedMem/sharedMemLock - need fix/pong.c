...
int AtomicChkAndDecNumOfMsgs(int* _isBreak, char* _mem, int _semId);
...
int ReadFromSharedMem(char* _mem , int _semId)
{
	int err, numOfMsgRcv = 0, isBreak = FALSE;
	...
	while(TRUE)
	{
		AtomicChkAndDecNumOfMsgs(&isBreak, _mem, _semId);
		if(isBreak){
			break;
		}
...
int AtomicChkAndDecNumOfMsgs(int* _isBreak, char* _mem, int _semId)
{
	...
	*_isBreak = FALSE;
	...
	/*--------------------------------------------*/
	/*need to be done atomicly - lock this section*/
	err = SemOp(_semId , LOCK , DOWN , NO_FLAGS );
	CHKERR(err);
	/*--------------------------------------------*/
	...
	/*--------------------------------------------*/
	/*unlock this section*/
	err = SemOp(_semId , LOCK , UP , NO_FLAGS );
	CHKERR(err);
	/*--------------------------------------------*/
